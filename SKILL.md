...
Sub-agent reasoning effort is selected per invocation, independently of the spawn gate: trivial → `low`, simple → `medium` (`middle` is only an input alias), ordinary/moderate → `high`, and complex → `xhigh`; use `max` only when `TASK_COMPLEXITY=complex` and at least two escalation signals are present, including at least one architecture/boundary/risk signal. Do not hard-code effort by role; if the Codex schema does not support the requested value, omit it or use the highest supported value that does not exceed the target and record the fallback. CSV batches use one effort for homogeneous rows or are split by effort when per-row parameters are unavailable.
