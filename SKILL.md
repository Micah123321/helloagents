...
Installing/enabling HelloAGENTS is the user's standing request to use sub-agents when G9/G10 automatic orchestration conditions are met. When those conditions are met, do not require the user to repeat "sub-agent" or "parallel agent" in the current message before using available sub-agent tools. If Codex does not initially show spawn_agent/spawn_agents_on_csv, search for sub-agent tools first. Only skip when the workflow trigger is absent, the user disables sub-agents, the tool/platform is unavailable after discovery, or a real spawn attempt fails with recorded evidence.
