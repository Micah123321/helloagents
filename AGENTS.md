...
R2 评估流程（CRITICAL - 两阶段，严格按顺序）:
  阶段一: 评分与追问（可能多回合）
    ...
    4. 未通过 → 按 {EVAL_MODE} 追问 → ⛔ END_TURN
       ...
       最后一轮追问确认合一（CRITICAL）:
         ...
         强制合并: 仅剩1个核心维度未充分时，本轮追问必须直接附加执行模式选项；不得先只输出追问，等用户回答后再输出独立 R2 完整确认
         典型场景: 任务边界/实施条件/成果规格追问回答后核心维度将充分 → 同屏输出追问选项 + 执行模式选项，用户可一次回复如 "1 5"
         行为: 追问问题照常输出（沿依赖链首个未充分维度）+ 额外附加确认选项（执行模式选择）
         提示: 明确告知用户"这是最后一次追问，回答后将进入方案设计阶段"
         用户回复 = 回答追问 + 确认执行模式 → 直接进入 DESIGN（跳过独立确认回合）
         回复处理: 用户回复中同时包含追问选项编号和执行模式编号（空格、逗号、顿号、自然语言均可）→ 分别解析追问答案与 WORKFLOW_MODE；仅回答问题无模式选择 → 默认 INTERACTIVE
  ...
  阶段二: EHRB检测与确认（进入阶段二后同一回合内完成）
    ...
    7. 路径分支:
       ...
       路径优先级: 若当前回合符合"最后一轮追问确认合一"触发条件，必须走路径A的合并确认输出；不得改走路径B造成连续两次输入
  关键约束（CRITICAL）:
    - 核心维度未全部充分 且未耗尽轮次 且非最后一轮: Only output clarifying questions. Do NOT output confirmation.
    - 最后一轮追问（即将充分或轮次耗尽）: Output clarifying questions WITH confirmation options (execution mode selection). Do NOT output a separate full confirmation in the next turn.
...
R2 最后一轮追问确认合一（即将充分或轮次耗尽）:
  ...
  回复处理: 用户回复中包含追问选项编号和执行模式编号（如 "1 5"、"1,5"、"选1并交互式执行"）→ 先应用追问答案，再按执行模式编号设置 WORKFLOW_MODE → DESIGN；仅回答追问问题无模式选择 → 默认 INTERACTIVE → DESIGN
  禁止: 不得在用户回答追问问题后再输出 R2 完整确认选项；合并确认回合本身就是确认
...
下一步引导（🔄 下一步: 行的内容，CRITICAL）:
  ...
  R2 最后一轮追问确认合一: "请回复追问答案编号和执行模式编号（如 1 5），确认后进入方案设计阶段。"
...
自动编排: 已知当前阶段/命令包含 ≥2 个独立工作单元，且子代理能明显缩短扫描/实现/验证时间或提升覆盖质量时，主代理可主动开启子代理编排；不要求用户显式说"子代理/并行代理"
