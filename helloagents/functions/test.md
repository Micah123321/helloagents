...
失败定位编排（按通用触发场景总表"测试/验证分析与失败定位"场景，应主动编排 [→ G10]）:
  触发: 失败测试文件 ≥2 或失败维度独立可并行（如多个互不相关的失败模块）
  行为: spawn worker/explorer 子代理并行读取失败测试 + 被测代码，独立定位每个失败/模块的根因（≤6/批）
  Codex 边界: 测试失败定位的 worker/explorer 默认不传 fork_context；prompt 必须自包含测试命令、失败输出摘要、失败文件、被测代码路径和分析边界
  主代理职责: 按 G10 有界等待收敛每个代理 → 汇总实际根因和 partial handoff → 只针对 pending_scope 生成统一修复建议
  不触发: 失败集中在单一文件/单一模块 → 主代理直接定位
  降级: 子代理失败 → 主代理直接定位 + tasks.md 标记 [降级执行]
```
