...
变更分析（按通用触发场景总表"提交辅助"场景，应主动编排 [→ G10]）:
  ...
  子代理调用:
    ...
    Codex explorer 调用默认不传 fork_context；prompt 必须包含待分析文件清单、diff 来源、目标说明和返回格式
...
预提交质量检查（finish-work，按通用触发场景总表"提交辅助"场景，应主动编排 [→ G10]）:
  子代理调用:
    ...
    Codex reviewer 调用默认不传 fork_context；prompt 必须包含暂存/候选提交范围、diff 摘要、检查维度和项目验证上下文；fork_context 仅在当前 schema 明确兼容时使用
    主代理汇总子代理返回的检查结果
    仅单一维度 → 主代理直接执行
