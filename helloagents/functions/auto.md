# ~auto 命令 - 委托执行

本模块定义委托执行命令的规则。用户确认后，AI 自动推进所有阶段。

---

## 命令说明

```yaml
命令: ~auto [<需求描述>]
类型: 完整流程类
功能: 从需求评估到开发实施的完整流程
目标模式: DELEGATED（用户确认后生效）
```

**DO:** 完成评估和 EHRB 检查后默认设置 WORKFLOW_MODE = DELEGATED，直接推进各阶段；各阶段仍按 G9/G10 规则调用子代理

**DO NOT:** 跳过需求评估或 EHRB 检查；不要把全自动与交互式重复做成每次都必须回答的菜单

---

## 回合A: 评估（收到 ~auto 命令时执行）

```yaml
当收到 ~auto [需求描述]:

  1. 设置 STAGE_ENTRY_MODE = NATURAL（WORKFLOW_MODE 暂不设置）
  2. 需求理解 + 评分 [→ G4 需求评估]

  核心维度未全部充分 → 追问回合（按 {EVAL_MODE}，含最后一轮追问确认合一）:
    输出追问信息 [→ G4 确认信息格式]
    ⛔ END_TURN（等待用户回复，用户回复后重新评分）

  核心维度全部充分 → 默认执行:
    3. EHRB 检测 [→ G2]
    4. WORKFLOW_MODE = DELEGATED，进入 DESIGN
```

---

## 回合B: 执行（用户确认后执行）

```yaml
当评估完成后:

  默认执行:
    WORKFLOW_MODE = DELEGATED
    按 R2 标准流程进入阶段链（见下方"阶段路由"）

  显式交互式执行:
    WORKFLOW_MODE = INTERACTIVE
    按 R2 标准流程进入阶段链，关键决策点 ⛔ END_TURN（方案选择、失败处理）

  交互式执行仅在用户明确提出“交互式执行”或“逐阶段确认”时启用；不存在额外模式确认页。
```

### 阶段路由

```yaml
R2 标准流程阶段链:
  1. 按 G7 按需读取表"R2 进入方案设计（入口）"加载 → 方案设计（含上下文收集，按 TASK_COMPLEXITY 决定是否多方案对比）
  2. 按 G7 按需读取表"R2 进入开发实施（入口）"加载 → 开发实施
  3. 验收: 按 G8 流程级验收规则执行
  4. 输出: 完成（验收报告+变更摘要+遗留提示，`🔄 下一步` 按 G3 "完成态下一步引导"规则生成）→ 状态重置
```

### DELEGATED 模式行为

```yaml
阶段间: 自动推进，不停顿
阶段内选择: 推荐选项自动选择
输出: 每个阶段仍输出摘要（禁止跳过输出）
overview 类型方案包: 直接归档，总结中标注

质量保障（CRITICAL）:
  子代理稳定性: 按 rules/subagent-protocols.md "Codex CLI 子代理稳定性策略" 执行
    等待预算: 按每个子代理的作用域单元数、依赖深度和任务类型动态计算，不使用全局固定等待时间
    任务级超时: 先强制请求 partial handoff，按动态宽限期等待，close 后主代理只接手 pending_scope
    连续 2 个子代理超时 → 进入主代理直接执行模式（不再尝试 spawn_agent）
    累计 ≥3 次 spawn→close 循环 → 进入主代理直接执行模式
  功能验收: develop 步骤9 在 DELEGATED 模式下不可简化，与 INTERACTIVE 模式执行完全相同的验证流程
  归档保障: develop 步骤14 为 CRITICAL 步骤，子代理失败时主代理必须接手完成
  省略范围: "省略中间态"仅指不在阶段间停顿等待确认，不意味着跳过任何验证/归档步骤
```

### INTERACTIVE 模式行为

```yaml
关键决策点: ⛔ END_TURN（方案选择、失败处理），评估确认后进入 DESIGN 阶段
阶段内选择: 输出选项 → ⛔ END_TURN → 等待用户选择
```

---

## 安全约束

```yaml
EHRB 检测始终生效，不受模式影响。
中断委托规则: [→ G5]
```

---

## 不确定性处理

| 场景 | 处理 |
|------|------|
| 需求描述不完整 | 回合A 中按 G4 追问循环处理 |
| 级别难以判定 | 默认使用 R2 标准流程（~auto 强制 R2） |
| 方案选择冲突 | DELEGATED 模式下选择推荐方案 |
| 测试失败但非阻断性 | 记录到验收报告，继续执行 |
