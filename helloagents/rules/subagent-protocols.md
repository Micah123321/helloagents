...
启用契约（CRITICAL）:
  ...
  complex 多方案构思: 计划 3~6 个 brainstormer，实际成功启动子代理数 ≥3 后才可继续多方案比较
  未触发: 最终可派发子代理数 <2 → 主代理直接执行，不调用单个子代理，不标记 `[降级执行]`
  能力前置: 工具、角色、平台和并发能力在最终可派发数达到门槛后检查；不可用属于编排失败降级，必须记录工具发现/环境证据，不得从候选数中剔除后伪装成“未触发”
  部分启动失败: 计划满足门槛但兼容重试后实际成功启动数低于本场景门槛 → 自动编排未启用；立即请求已启动代理 partial handoff，并按当前 CLI 的停止/回收协议确认其不再扩展；无显式 close 能力时必须等待可验证终态或确认停止，无法确认时阻断重叠接管并记录能力限制。保留有证据的完成范围，主代理只接手 pending_scope，失败项记录可审计降级证据
...
强制调用规则（标注"强制"的必须调用，标注"跳过"的可跳过，子代理编排均遵循自动编排原则 + 通用触发场景总表）:
  ...
  DESIGN:
    ...
    Phase2（方案构思）—
    brainstormer — R2 标准流程步骤10 方案构思时强制（TASK_COMPLEXITY=complex），计划 3~6 个且实际成功启动 ≥3 个子代理后进入多方案比较（每个独立构思一个方案）
...
子代理调用合规检查（阶段验收时执行）:
  自动编排合规:
    DESIGN 阶段:
      检查: brainstormer 实际成功启动数 ≥3 — 条件: TASK_COMPLEXITY=complex
    DEVELOP 阶段:
