...
主动触发: 已知当前阶段或命令包含 ≥2 个独立工作单元，且子代理能明显缩短扫描/实现/验证时间或提升覆盖质量时，主代理可主动开启子代理编排；不要求用户显式说"子代理/并行代理"
...
强制调用规则（标注"强制"的必须调用，标注"跳过"的可跳过，子代理编排均遵循自动编排原则）:
  ...
  命令路径:
    ~review: 子代理（按编排五步法选择类型）— ≥2个分析维度或审查文件≥2 且并行收益明确时，可按 functions/review.md 的当前分析维度或文件分组主动并行；确认、EHRB、阻塞等待、结果真实性和主代理汇总边界不变
    ...
    其他命令: 已知命令流程内出现 ≥2 个独立扫描/实现/验证单元，且并行收益明确时，可按自动编排原则主动编排；命令确认、EHRB 和写入职责边界不变
