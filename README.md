...
3 个专业角色（reviewer / writer / brainstormer）+ 宿主 CLI 原生子代理（explore / code / brainstorm），根据独立工作单元和并行收益自动调度，不要求用户显式说“子代理/并行代理”。语义过滤后至少 2 个可派发且至少 2 个实际启动时才启用自动编排；不足 2 个不 spawn，由主代理执行。若仅启动 1 个，则请求 handoff，并按宿主支持的停止/回收机制收敛；没有显式 close API 时必须等待可验证终态或确认停止后再接管，两者均不可用则阻断重叠接管并记录能力限制。工具或平台不可用属于门槛成立后的编排失败，不参与候选过滤。复杂方案构思要求至少 3 个 brainstormer 实际启动，并至少返回 3 个代理独立生成的可用方案后才进入比较。任务通过 DAG 依赖分析进行拓扑排序，按层并行派发。
...
`~auto` 或通用 R2 任务在评估通过后默认进入委托模式；不再重复弹出执行模式选择：

- **委托模式（默认）：** 自动推进所有阶段，自动选择推荐选项，仅在真实歧义、EHRB 风险或阻断性失败时暂停
- **交互模式（显式选择）：** 用户明确提出“交互式执行”或“逐阶段确认”后，在关键决策点暂停
- **仅规划委托：** `~plan` 固定进入委托规划，完成设计后停止，不进入开发
...
最后一轮只处理仍然缺失的需求信息；评估通过后默认进入全自动阶段链，不再追加“交互式/全自动”选择。已有代码库中可推断的上下文会自动计入评分。如果需求已经足够明确，可以说“跳过评估 / 直接做”跳过追问环节；只有真实歧义、EHRB 风险或阻断性失败才会暂停。

### 设计阶段并行方案

只有 `TASK_COMPLEXITY=complex` 且确实需要多方案比较时，设计阶段才计划派发 3–6 个子代理，各自独立生成竞争性实现方案。主代理从用户价值、方案合理性、风险（含 EHRB）、实现成本四个维度评估所有方案，权重根据项目特征动态调整（如性能敏感项目提高合理性权重，MVP 项目提高成本权重）。
...
除自动调度外，可手动指定角色执行任务；显式调用单个角色属于手动委派，不属于自动编排：
